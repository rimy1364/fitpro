...
export default async function DietPlansPage({ searchParams }: { searchParams: Promise<{ assignTo?: string }> }) {
  const session = await auth();
  const trainerId = session!.user.id;
  const params = await searchParams;

  const [dietPlans, myTrainees] = await Promise.all([
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Create and manage diet plans for your trainees</p>
        </div>
        <Link href="/trainer/diet-plans/new">
      ...
      {dietPlans.length === 0 ? (
        <Card>
          <CardContent className="py-16 text-center">
            ...
            <p className="text-gray-500 mb-4">No diet plans created yet.</p>
            <Link href="/trainer/diet-plans/new">
            ...
            <Card key={plan.id} className="hover:shadow-md transition-shadow">
              <CardContent className="p-5">
                ...
                <div className="flex items-center gap-3 text-xs text-gray-500 mb-4">
                  <span>{plan._count.meals} meals</span>
                  <span>·</span>
                ...
                <div className="flex gap-2">
                  <Link href={`/trainer/diet-plans/${plan.id}`} className="flex-1">
